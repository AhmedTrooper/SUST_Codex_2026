...
const SAMPLES = {
	...
	failed_payment: {
		ticket_id: "TKT-002",
		complaint:
			"I tried to pay my bill of 2300 BDT yesterday at the merchant shop but the payment failed. However, my account balance was still deducted. Please refund my money.",
		language: "en",
		channel: "in_app_chat",
		user_type: "customer",
		campaign_context: "boishakh_bonanza_day_1",
	...
	phishing: {
		ticket_id: "TKT-003",
		complaint:
			"I received a call from someone claiming to be from your customer support. They told me there was a security issue with my account and asked for my PIN and OTP. Is this official?",
		language: "en",
		channel: "in_app_chat",
		user_type: "customer",
		campaign_context: "boishakh_bonanza_day_1",
...
function Home() {
	...
	const [campaignContext, setCampaignContext] = React.useState(
	...
	const fetchHistory = async (offsetVal: number) => {
		...
		try {
			const res = await fetch(
				`${apiBaseUrl}/tickets?limit=${historyLimit}&offset=${offsetVal}`,
			);
			if (res.ok) {
	...
	const loadSample = (
		type: "wrong_transfer" | "failed_payment" | "phishing",
	) => {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		for (let i = 0; i < transactions.length; i++) {
			...
			if (t.amount <= 0 || isNaN(t.amount)) {
				setError(
					`Transaction #${i + 1} (${t.transaction_id}) amount must be a positive number greater than 0 BDT.`,
				);
				return;
			}
			if (!t.counterparty.trim()) {
				setError(
					`Transaction #${i + 1} (${t.transaction_id}) is missing a Counterparty ID or Phone Number.`,
				);
				return;
			}
		}
		...
		try {
			...
			setError(
				"Could not connect to the backend API. Please make sure the backend is running.",
			);
		} finally {
			setIsLoading(false);
		}
			...
			<section className="mb-12 border-b border-[var(--line)] pb-8 text-left rise-in">
				<div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
					...
					<div className="flex flex-wrap gap-2">
						...
						<span className="font-mono text-[11px] self-center text-[var(--sea-ink-soft)] mr-1">
							Load Case:
						</span>
						<Button
							variant="outline"
				...
				<section className="mb-8 p-5 border border-black dark:border-white rounded-lg bg-[var(--foam)] text-left relative rise-in">
					...
					<div className="grid grid-cols-1 md:grid-cols-5 gap-4 font-mono text-[11px] leading-relaxed text-[var(--sea-ink-soft)]">
						<div className="p-3 border border-[var(--line)] bg-[var(--chip-bg)]/20 rounded relative">
							...
							<h4 className="font-bold text-[var(--sea-ink)] m-0 mb-1">
								Load Sample Case
							</h4>
							<p className="m-0">
								Click any sample button in the top-right toolbar (e.g.{" "}
								<i>Wrong Transfer</i> or <i>Failed Payment</i>) to load mock
								data.
							</p>
						</div>
						<div className="p-3 border border-[var(--line)] bg-[var(--chip-bg)]/20 rounded relative">
							...
							<p className="m-0">
								Analyze the transaction records. Use the <i>Add Txn</i> sub-form
						...
						<div className="p-3 border border-[var(--line)] bg-[var(--chip-bg)]/20 rounded relative">
							...
							<p className="m-0">
								Click <i>Run Investigation Pipeline</i>. The system checks data
						...
						<div className="p-3 border border-[var(--line)] bg-[var(--chip-bg)]/20 rounded relative">
							...
							<p className="m-0">
								Inspect routed department, evidence verdict, and the safe
								customer reply draft (PIN/OTP requests are automatically
								blocked).
							</p>
						</div>
						<div className="p-3 border border-[var(--line)] bg-[var(--chip-bg)]/20 rounded relative">
							...
							<p className="m-0">
								The resolved ticket is automatically saved in Postgres and
								loaded into the historical log list at the bottom.
							</p>
						</div>
					</div>
				</section>
						...
						<div className="border-t border-[var(--line)] pt-4">
							...
							{transactions.length === 0 ? (
								<div className="border border-dashed border-[var(--line)] p-4 text-center rounded text-xs font-mono text-[var(--sea-ink-soft)] bg-[var(--chip-bg)]/20">
									No transactions added. Analysis will fall back to
									safety-checks and rules evaluating text only.
								</div>
							) : (
								<div className="space-y-3 max-h-[220px] overflow-y-auto pr-1">
									{transactions.map((txn, index) => (
										<div
											key={index}
											className="p-3 border border-[var(--line)] bg-[var(--chip-bg)]/30 rounded relative flex flex-col gap-2"
										>
											...
											<div className="grid grid-cols-3 gap-2">
												...
												<select
													value={txn.status}
													onChange={(e) =>
														updateTransaction(index, "status", e.target.value)
													}
					...
					{!result ? (
						<div className="flex-1 flex flex-col items-center justify-center text-center p-8 border border-dashed border-[var(--line)] bg-[var(--chip-bg)]/10 rounded">
							...
							<p className="text-xs font-mono text-[var(--sea-ink-soft)] max-w-sm m-0">
								Submit a support ticket on the left panel. The copilot will
								cross-examine the text history and render evidence findings
								instantly.
							</p>
						</div>
					) : (
						<div className="space-y-5 text-left flex-1 flex flex-col">
							<div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
								<div className="p-2.5 border rounded border-[var(--line)] bg-[var(--foam)] text-center">
									...
									<span
										className={`inline-block px-2 py-0.5 border text-[10px] font-bold uppercase rounded ${getVerdictStyle(result.evidence_verdict)}`}
									>
								...
								<div className="p-2.5 border rounded border-[var(--line)] bg-[var(--foam)] text-center">
									...
									<span
										className={`inline-block px-2 py-0.5 border text-[10px] font-bold uppercase rounded ${getSeverityStyle(result.severity)}`}
									>
										{result.severity ?? "low"}
									</span>
								</div>
							...
							<div className="grid grid-cols-2 gap-4 p-3 border border-[var(--line)] bg-[var(--chip-bg)]/20 rounded font-mono text-[11px]">
								<div className="flex flex-col">
									<span className="text-[var(--sea-ink-soft)] uppercase text-[9px] tracking-wider mb-0.5">
										Matched Transaction
									</span>
									<span className="font-bold text-[var(--sea-ink)]">
										{result.relevant_transaction_id ? (
											<code className="text-xs bg-[var(--chip-bg)] border border-[var(--line)]">
												{result.relevant_transaction_id}
											</code>
										) : (
											"None"
								...
								<div className="flex flex-col">
									...
									<span
										className={`font-bold ${result.human_review_required ? "text-amber-500" : "text-[var(--sea-ink-soft)]"}`}
									>
					...
					<table className="w-full border-collapse text-left text-xs font-mono">
						<thead>
							<tr className="border-b border-[var(--line)] bg-[var(--chip-bg)]/50 text-[var(--sea-ink-soft)]">
								...
								<th className="p-3 font-semibold uppercase tracking-wider max-w-[200px] truncate">
									Complaint
								</th>
								<th className="p-3 font-semibold uppercase tracking-wider text-center">
									Verdict
								</th>
								<th className="p-3 font-semibold uppercase tracking-wider text-center">
									Case Type
							...
							{isHistoryLoading ? (
								...
								<tr>
									<td
										...
										className="p-8 text-center text-[var(--sea-ink-soft)]"
									>
										No analyzed tickets found in the database. Tickets analyzed
										using "Run Investigation Pipeline" will be saved and
										displayed here.
									</td>
								</tr>
							) : (
								history.map((ticket) => (
									...
									>
										...
										<td
											className="p-3 max-w-[200px] truncate font-sans text-xs"
										...
										<td className="p-3 text-center">
											<span
												className={`inline-block px-1.5 py-0.5 border text-[9px] font-bold uppercase rounded ${getVerdictStyle(ticket.evidence_verdict)}`}
											>
												{ticket.evidence_verdict}
											</span>
										</td>
										<td className="p-3 text-center font-bold text-[var(--sea-ink)]">
											{ticket.case_type}
										</td>
										<td className="p-3 text-center">
											<span
												className={`inline-block px-1.5 py-0.5 border text-[9px] font-bold uppercase rounded ${getSeverityStyle(ticket.severity)}`}
											>
												{ticket.severity}
											</span>
										</td>
